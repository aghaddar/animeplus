      ...
      <div className="relative">
        ...
        <div className="hidden md:flex items-center space-x-4 absolute left-4 top-0 bottom-0">
          ...
          <div
            className="relative"
            onMouseEnter={() => {
              if (genreCloseTimeout.current) clearTimeout(genreCloseTimeout.current)
              setIsGenreOpen(true)
            }}
            onMouseLeave={() => {
          ...
          >
            <button className="nav-link flex items-center gap-1 text-white hover:text-purple-400 focus:outline-none group" type="button">
              Genre
              <ChevronDown className={`w-4 h-4 transition-transform duration-500 ml-1 ${isGenreOpen ? "rotate-180" : ""}`} />
            </button>

            <div
              className={`absolute left-0 mt-2 w-[800px] bg-gray-900 rounded-2xl shadow-lg z-50 py-2 px-4 transition-all duration-500 ${
                isGenreOpen ? "opacity-100 translate-y-0 pointer-events-auto" : "opacity-0 translate-y-2 pointer-events-none"
              }`}
            ...
            >
              {(() => {
                ...
                const grid = Array.from({ length: rows }, (_, rowIdx) => genres.slice(rowIdx * columns, rowIdx * columns + columns))
                return (
        ...
        {/* Right: notifications + profile/login (anchored right on md+) */}
        <div className="hidden md:flex items-center space-x-4 absolute right-4 top-0 bottom-0 justify-end">
          ...
          {!isLoading && (
            <>
              {user ? (
                <div className="relative">
                  <button onClick={toggleProfile} className="flex items-center justify-center w-8 h-8 rounded-full overflow-hidden border-2 border-purple-500 bg-purple-600">
                    {user.avatarURL ? (
                      <Image
                  ...
                  <UserProfileDropdown isOpen={isProfileOpen} onClose={() => setIsProfileOpen(false)} userName={user.username} profileImage={user.avatarURL} />
                </div>
              ) : (
                <Link href="/auth/login" className="hidden md:flex items-center text-white hover:text-purple-400 transition-colors group">
          ...
          <div className="md:hidden bg-gray-900 mt-2 rounded-lg overflow-hidden shadow-lg">
            <div className="p-4">
              ...
              <nav className="flex flex-col space-y-3">
                <div className="relative">
                  <button type="button" className="flex items-center w-full px-3 py-2 rounded-md text-white hover:bg-gray-800 focus:outline-none group" onClick={() => setIsGenreOpen((v) => !v)}>
                    Genre
                    <ChevronDown className={`w-4 h-4 ml-1 transition-transform duration-500 ${isGenreOpen ? "rotate-180" : ""}`} />
                  </button>
                ...
                {!user && (
                  <Link href="/auth/login" className="flex items-center px-3 py-2 rounded-md text-white hover:bg-gray-800 group">
