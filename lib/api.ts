...
export function proxyHlsUrl(url: string): string {
  if (!url) return url
...
function normalizeZoroAnime(zoroAnime: any): AnimeResult {
  return {
    ...
    description: zoroAnime.synopsis || zoroAnime.description || "",
...
export async function getFeaturedAnime(): Promise<AnimeResult[]> {
  ...
  try {
    ...
    if (data && data.results && Array.isArray(data.results)) {
      ...
      // Get the first 3 results and fetch detailed info for each to get descriptions
      const topAnime = data.results.slice(0, 3).map(normalizeZoroAnime)
      
      // Fetch detailed information for each anime to get descriptions
      const animeWithDescriptions = await Promise.all(
        topAnime.map(async (anime: AnimeResult) => {
          try {
            const detailResponse = await fetch(`${API_BASE_URL}${PROVIDER_PATH}/info/${anime.id}`, {
              next: { revalidate: 300 }, // Cache for 5 minutes
            })
            
            if (detailResponse.ok) {
              const detailData = await detailResponse.json()
              const normalizedDetail = normalizeZoroAnime(detailData)
              return {
                ...anime,
                description: detailData.synopsis || normalizedDetail.description || detailData.description || "An exciting anime series with captivating story and amazing characters. Watch now to discover more!",
              }
            }
          } catch (error) {
            console.warn(`Failed to fetch description for anime ${anime.id}:`, error)
          }
          
          // Fallback if detail fetch fails
          return {
            ...anime,
            description: "An exciting anime series with captivating story and amazing characters. Watch now to discover more!",
          }
        })
      )
      
      return animeWithDescriptions
    } else {
